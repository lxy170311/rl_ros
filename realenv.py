...
class RealEnv(Node):
    def __init__(self, name, environment_dim):
        ...
        self.pump_control = 0.0 #预设值，训练期间水泵输出不变
        ...
        self.pump_publisher = self.create_publisher(
            Float32MultiArray, "/water_pump", 10
        )

        # 订阅话题
        # self.odom_subscriber = self.create_subscription(Odometry, '/odom', self.odom_callback, 10)
        self.imu_subcriber = self.create_subscription(
            Imu, "/imu", self.imu_callback, 10
        )
        self.sonar_subscriber = self.create_subscription(
            Range, "/range_sensor", self.sonar_callback, 10
        )
    ...
    def imu_callback(self, msg):
        # 将四元数转成欧拉角
        (row, pitch, yaw) = tf.transformations.euler_from_quaternion(
